...
describe Cache do
	...
	it 'Sets a value in the cache normally' do
		expect(cache.set('1',30,1000,200,'Test',false)).to eq(STORED_MESSAGE)
	end
	...
	it 'Adds a value in the cache normally' do
		expect(cache.add('1',30,1000,200,'Test',false)).to eq(STORED_MESSAGE)
	end
	it 'Adds a value in the cache and it already exists' do
		cache.set('1',30,1000,200,'Test',false)
		expect(cache.add('1',30,1000,200,'Test',false)).to eq(NOT_STORED_MESSAGE)
	end
	...
	it 'Replaces a value in the cache normally' do
		cache.set('1',30,1000,200,'Test',false)
		expect(cache.replace('1',30,1000,200,'Test',false)).to eq(STORED_MESSAGE)
	end
	it 'Replaces a value in the cache and it does not exist' do
		expect(cache.replace('1',30,1000,200,'Test',false)).to eq(NOT_STORED_MESSAGE)
	end
	...
	it 'Appends a value to an already stored value in the cache' do
		cache.set('1',30,1000,200,'Test',false)
		expect(cache.append('1',30,1000,200,'Ok',false)).to eq(STORED_MESSAGE)
	end
	it 'Appends a value in the cache and it does not exist' do
		expect(cache.append('1',30,1000,200,'Ok',false)).to eq(NOT_STORED_MESSAGE)
	end
	...
	it 'Prepends a value to an already stored value in the cache' do
		cache.set('1',30,1000,200,'Test',false)
		expect(cache.preppend('1',30,1000,200,'Ok',false)).to eq(STORED_MESSAGE)
	end
	it 'Prepends a value in the cache and it does not exist' do
		expect(cache.preppend('1',30,1000,200,'Ok',false)).to eq(NOT_STORED_MESSAGE)
	end
	...
	it 'Cas a value to an already stored value in the cache' do
		cache.set('1',30,1000,200,'Test',false)
		cache.gets(['1'])
		expect(cache.cas('1',30,1000,200,'Ok',1,false)).to eq(STORED_MESSAGE)
	end
	it 'Cas a value in the cache and it was updated' do
		cache.set('1',30,1000,200,'Test',false)
		cache.gets(['1'])
		cache.gets(['1'])
		expect(cache.cas('1',30,1000,200,'Ok',1,false)).to eq(EXISTS_MESSAGE)
	end
	...
	#####Tests for get#######
	#########################
	it 'Get a value from the cache normally' do
		cache.set('1',30,1000,200,'Test',false)
		expect(cache.get(['1'])).to eq("Test 1 30 200\r\nEND\r\n")
	end
	it 'Get multiple values from the cache normally' do
	...
	it 'Get a value from the cache after replacing' do
		cache.set('1',30,1000,200,'Test',false)
	...
	it 'Get a value after appending' do
		...
		expect(cache.get(['1'])).to eq("TestOk 1 30 400\r\nEND\r\n")
	end
	it 'Get a value after prepending' do
		cache.set('1',30,1000,200,'Test',false)
		cache.preppend('1',30,1000,200,'Ok',false)
		expect(cache.get(['1'])).to eq("OkTest 1 30 400\r\nEND\r\n")
	end
